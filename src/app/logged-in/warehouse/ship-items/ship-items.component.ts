...
@Component({
  selector: 'app-ship-items',
  templateUrl: './ship-items.component.html',
  styleUrls: ['./ship-items.component.css']
})
export class ShipItemsComponent implements OnInit {
...
console.log(formData);
  ...
  ServiceCallSubscribe(){
    ...
    //         this.getInvoiceInformations(this.accountId);
    //         if(machineid > 0){
  ...
  salesInfoList : any =[]
  getAllInoviceInformation(accountId){
    this.onLineList = [];
    this.showDataTable = false;
    this.utils.StartSpinner();
    this.context.getWithToken('/'+accountId+'/'+0+'/'+4+'/'+0,'sales/getaccountuserssales').
      subscribe( data => {
        let d = <any>data;
        this.salesInfoList = d.data;
        // if(d.data == null){
        //   this.salesInfoList = {};
        // }   
        console.log(d)
